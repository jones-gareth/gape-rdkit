...
namespace Gape {
...
#ifndef NDEBUG
        assert(equals(dummy2[0], 0, 1e-10));
        assert(equals(dummy2[1], 0, 1e-10));
        assert(dummy2[2] < 0);
#endif
    ...
    void AcceptorAtom::addOnePairToAtom(const Conformer &conformer, std::vector<RDGeom::Point3D> &lonePairs) const {
        switch (atom->getDegree()) {
            ...
            default:
                throw std::domain_error(
                    (boost::format("addOnePairToAtom: Unable to add a single LP to atom %d with degree %d") % atom->
                     getIdx()
                     % atom->getDegree()).str()
                );
        }
    ...
    bool AcceptorAtom::checkLonePairLengths(const Conformer &conformer,
                                            const std::vector<RDGeom::Point3D> &lonePairs) const {
        ...
        if (numberLonePairs != static_cast<int>(lonePairs.size())) {
            REPORT(Reporter::WARN) << "Lone pair count error: expected " << numberLonePairs << " got " << lonePairs.
size();
            return false;
        }
